...
def extract_match_features(match, time_point=None):
    ...
    team_items_radiant = np.zeros(255)
    team_items_dire = np.zeros(255)
    ...
    for player_index, player in enumerate(match['players']):
        ...
        for entry in filter_events(player['purchase_log']):
            item_id = entry['item_id']
            if item_id > 254:
                continue
            if 'r' in player_id:
                team_items_radiant[item_id] = 1
                continue
            team_items_dire[item_id] = -1

    for i in xrange(len(team_items_radiant)):
        item = 'radiant_item_' + str((i))
        feats += [(item, team_items_radiant[i])]
    for i in xrange(len(team_items_dire)):
        item = 'dire_item_' + str((i))
        feats += [(item, team_items_dire[i])]
