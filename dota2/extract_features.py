...
def extract_match_features(match, time_point=None):
    ...
    player_items = []
    for i in xrange(10):
        player_items.append(np.zeros(255))
    ...
    for player_index, player in enumerate(match['players']):
        ...
        for entry in filter_events(player['purchase_log'], time_point):
            ...
            if item_id > 254:
                continue
            if 'r' in player_id:
                player_items[player_index][item_id] = 1
                continue
            player_items[player_index][item_id] = -1

    for i in xrange(len(player_items)):
        player_id = ('r%d' % (i+1)) if i < 5 else ('d%d' % (i-4))
        p_items = player_items[i]
        for j in xrange(len(p_items)):
            feats += [(player_id + '_item_' + str(j), p_items[j])]
...
def iterate_matches(matches_filename):
    ...
    with open(matches_filename) as f:
